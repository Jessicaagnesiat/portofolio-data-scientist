...
document.addEventListener("DOMContentLoaded", () => {
  const texts = ["Data Analyst", "Data Scientist"];
  let textIndex = 0;
  let charIndex = 0;
...
contactForm.addEventListener('submit', function (e) {
    ...
    if (!name || !email || !message) {
        alert("Nama, Email, dan Message wajib diisi ya 🙂");
        return;
    }

    const subject = encodeURIComponent(`Portfolio Contact: ${project || "-"}`);
    const body = encodeURIComponent(
        `Nama: ${name}\nEmail: ${email}\n\nMessage:\n${message}`
    );

    const gmailURL =
        `https://mail.google.com/mail/?view=cm&fs=1&to=jessicaagnesiat@gmail.com&su=${subject}&body=${body}`;
...
filterButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // 🔥 WAJIB: reset swiper setelah filter
